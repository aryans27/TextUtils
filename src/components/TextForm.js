...
export default function TextForm(props) {
    ...
    const handleCopy = () => {
        ...
        // document.getSelection().removeAllRanges()        // do this or do the other one       // this will unselect the text from the textarea
    ...
    return (
        <>
            ...
            <div className="container my-3" style={{ color: props.mode === 'dark' ? 'white' : '#042743' }}>
                ...
                {/* to split words by space or new line charecter, use /\s+/ */}
                <p>{text.split(/\s+/).filter((element) => { return element.length !== 0 }).length} words and {text.length} charecters.</p>
                <p>{0.008 * text.split(' ').filter((element) => { return element.length !== 0 }).length}min taken to read.</p>
                <h1>Preview</h1>
